...
def checkout(request):
    ...
    return render(request, 'checkout.html', {'cart': cart, 'total': cart.get_total_price()})
...
def admin_book_add(request):
    ...
    if request.method == 'POST':
        ...
        form = BookForm()
    return render(request, 'book_form.html', {'form': form, 'action': 'Thêm sách'})
...
def admin_book_edit(request, book_id):
    ...
    return render(request, 'book_form.html', {'form': form, 'action': 'Chỉnh sửa sách'})
